	...
	*/
	...
	public void decode(FacesContext context)
	{
		...
		if (this.isRendered())
		{
		    ...
		    Boolean isFeatureSelectedEvent  = false;
		    ...
		    if (null!=behaviorEvent)
		    {
			    ...
			    isFeatureSelectedEvent  = behaviorEvent.equals("featureSelected");
		    }
		    ...
		    if (null!=serviceList && dmMap!=null && dmMap.getViews()!=null)
			{
		    	...
		    	for (VIEW view : dmMap.getViews())
				{
					...
					Boolean before    = serviceList.get(serviceId).isLayerVisible(layerId);
		...
		if (null!= behaviorEvent && isUpdateModelEvent)
		{
	        ...
	        if (null!= behaviorEvent && isFeatureSelectedEvent)
	        {
	        	java.util.Map<String, List<ClientBehavior>> behaviors = getClientBehaviors();
	     		if (behaviors.isEmpty())
	     		{
	            ...
	            if (behaviors.size() > 0)
	            {
	            	...
	            	if (behaviorSource != null && null!= behaviorsForEvent && behaviorSource.equals(clientId))
	            	{
	            		for (ClientBehavior behavior: behaviorsForEvent)
	            		{
	            			logger.info("Found isFeatureSelectedEvent: " +behavior.getClass().toString());

	            			MapAjaxEvent ajaxEvent = new MapAjaxEvent(this, behavior);

	            			logger.trace("Setting Selected feature ");
	            			ajaxEvent.setSelectedFeature(params);
	            			behavior.broadcast(ajaxEvent);
	            		}
	            	}
	            }
